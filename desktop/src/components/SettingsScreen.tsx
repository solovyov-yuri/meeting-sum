...
function TranscriptionSection({ draft, update }: { draft: AppSettings; update: UpdateFn }) {
  ...
  return (
    <div>
      ...
      <p className="mt-4 rounded-card border border-border bg-panel-soft p-3.5 text-sm text-ink-muted">
        При первом запуске с устройством «CUDA (GPU)» библиотеки CUDA (~1.7 ГБ) скачаются
        автоматически — с прогрессом и возможностью отмены. Без них доступен режим CPU.
      </p>
    </div>
  );
}
